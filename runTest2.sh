...
nohup sh apache-jmeter-5.5-SNAPSHOT/bin/jmeter.sh -Jjmeter.save.saveservice.output_format=csv -n -t ./$scriptName -f -l ./Test$time.jtl -e -o ./HTMLReport_$time > ./$results/nohup.txt 2>&1
sh apache-jmeter-5.5-SNAPSHOT/bin/JMeterPluginsCMD.sh --generate-csv ./Aggregate_$time.csv --input-jtl ./Test$time.jtl --plugin-type AggregateReport
